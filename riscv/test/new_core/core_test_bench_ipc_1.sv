module core_test_bench_ipc_1;
    ...
    initial begin
        load_program("/tmp/bench_ipc_1.hex");
        run_program(5000000);

        $display("bench_ipc_1: %0d cycles, %0d instrs, IPC=%0d.%02d (1 iter, N=32, cold)",
            cycle_count, instr_count,
            instr_count / cycle_count,
            (instr_count * 100 / cycle_count) % 100);

        if (cycle_count >= 5000000) begin
            $display("bench_ipc_1: TIMEOUT");
            errors++;
        end

        if (errors == 0) $display("bench_ipc_1: PASSED");
        else $display("bench_ipc_1: FAILED (%0d errors)", errors);
        $finish;
    end
endmodule
