...
- .text: **5264 байт** (~5.2 КБ)
- .bss: 536 байт
- Загрузка из flash при 5 МГц: ~8 мс

### Что делает

1. Проверяет наличие SD карты (card detect)
2. Инициализирует SD (CMD0 → CMD8 → ACMD41 → CMD58, SDHC/SDSC)
3. Читает MBR, находит FAT32 раздел
4. Парсит BPB, вычисляет геометрию FAT
5. Ищет `BOOT    BIN` в корневой директории (8.3 format)
6. Читает файл по цепочке кластеров в DDR по адресу 0x00000000
7. Прыгает на 0x00000000
