...
| Метрика | Описание | S | B | B+S | B+I4 | B+I16 |
|---------|----------|---|---|-----|------|-------|
| setup | Камера, очистка буферов | 206 | 242 | 231 | 157 | 157 |
| bsp | BSP traversal + стены | 80,930 | 66,130 | 67,090 | 49,980 | 50,090 |
| planes | Полы/потолки (visplane+span) | 99,400 | 25,250 | 26,790 | 29,220 | 29,310 |
| masked | Спрайты, прозрачные текстуры | 7,800 | 6,020 | 6,150 | 3,980 | 3,980 |
| **RENDER** | **Итого рендер** | **188,350** | **97,640** | **100,260** | **83,340** | **83,540** |

## Full frame breakdown (us)

| Метрика | Описание | S | B | B+S | B+I4 | B+I16 |
|---------|----------|---|---|-----|------|-------|
| pre | Статус-бар, меню, wipe | 3,760 | 3,710 | 3,710 | 1,420 | 1,420 |
| render | Рендер (см. выше) | 188,350 | 97,640 | 100,260 | 83,340 | 83,540 |
| post | HUD-текст, рамка | 39 | 40 | 40 | 32 | 32 |
| oled | Downscale + SPI flush | 7,300 | 7,330 | 7,340 | 2,400 | 2,400 |
| tics | AI, физика, движение | 11,540 | 11,360 | 11,190 | 6,010 | 5,970 |
| input | Чтение UART | 0 | 0 | 1 | 0 | 0 |
| **LOOP** | **Полный кадр** | **226,000** | **135,000** | **137,600** | **107,900** | **108,000** |
| **FPS** | **Кадров в секунду** | **4.4** | **7.4** | **7.3** | **9.3** | **9.3** |
...
| Метрика | B | B+I4 | Ускорение | Причина |
|---------|---|------|-----------|---------|
| bsp | 66,130 | 49,980 | **-24%** | BSP-цикл в I-cache, D-cache не вытесняется |
| planes | 25,250 | 29,220 | +16% (хуже) | Не conflict miss (16KB не помог) — см. анализ ниже |
...
### Planes regression: не размер I-cache

B+I16 (16 KB) даёт те же planes что B+I4 (4 KB): ~29,300 us vs 29,220 us.
Значит regression **не вызвана** conflict miss в I-cache.

Возможные причины:
- D-cache (4-pool, 64B) теперь полностью занят данными (раньше часть линий содержала
  инструкции, и при planes D-cache "случайно" содержал нужные данные)
- Изменение паттерна D-cache eviction из-за отсутствия instruction pressure
- Блиттер читает текстуры через D-cache (bus_type=00), I-cache не влияет на эту часть

## Текущие узкие места (B+I4)
...
| 5 | Увеличить D-cache (8-16 pool) | bsp, planes | Меньше data miss |
