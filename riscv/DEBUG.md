...
| Состояние | Описание |
|-----------|----------|
| **S_IDLE** | Ждёт `rx_valid`. Debug-команды (0x01–0x07) → pipeline. Остальные → CPU passthrough. CPU TX → `0xBB` + байт (через S_CPU_TX). |
| **S_RECV** | Приём payload побайтно: первые 4 → `payload_addr`, следующие 4 → `payload_data` (little-endian). 0xFD здесь — данные, не сброс. |
| **S_PAUSE_WAIT** | `bus_request=1`. Ждёт `dbg_bus_granted`. |
...
2. `cpu_tx_valid=1` → DEBUG_CONTROLLER (в S_IDLE)
3. Отправляет: `0xBB` + байт
...
2. **CPU TX только в S_IDLE.** Если контроллер обрабатывает debug-команду, CPU TX буферизируется TX FIFO.

3. **INPUT останавливает pipeline.** Доставка байта в CPU идёт через стандартный pipeline (pause → exec → ACK). Байт сохраняется в UART_IO_DEVICE.rx_data_r и ждёт чтения CPU.

4. **0xFD в payload.** Если команда ожидает payload (READ_MEM, WRITE_MEM, INPUT, RESET_PC), байт 0xFD воспринимается как данные, не как SYNC_RESET.
